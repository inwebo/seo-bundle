    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        $container->services()
            ->set('inwebo_seo.bread', Bread::class)
                ...
                ->arg('$entityManager', service(EntityManagerInterface::class))
                ->arg('$bag', service(BreadcrumbBag::class))
                ->arg('$entityFQCN', $config[Entity\Breadcrumb::CONFIG_KEY]['entity'])
        ...
        $container->services()
            ->set('inwebo_seo.metadata', Metadata::class)
                ->arg('$environment', service(Environment::class))
                ->arg('$entityManager', service(EntityManagerInterface::class))
                ->arg('$bag', service(MetadataBag::class))
                ->arg('$entityFQCN', $config[Entity\Metadata::CONFIG_KEY]['entity'])
                ->arg('$excludedRoutes', $config[Entity\Metadata::CONFIG_KEY]['excluded_routes'])
                ->tag('kernel.event_subscriber')
                ->autowire(false)
        ...
        $container->services()
            ->set('inwebo_seo.bag.breadcrumbs', BreadcrumbBag::class)
            ->alias(BreadcrumbBag::class, 'inwebo_seo.bag.breadcrumbs')
            ->public()
        ;

        $container->services()
            ->set('inwebo_seo.bag.metadata', MetadataBag::class)
            ->alias(MetadataBag::class, 'inwebo_seo.bag.metadata')
            ->public()
        ;
    }
}
